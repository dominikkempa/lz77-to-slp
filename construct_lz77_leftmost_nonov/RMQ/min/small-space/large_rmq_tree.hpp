  ...
  public:
    ...
    inline bool less(std::uint64_t beg, std::uint64_t end,
      ...
      while (left + 1 != right) {
        if (!(left & 1)) {
          if (left >= off)
            ret = std::min(ret, (std::uint64_t)m_tab[left - off + 1]);
          else ret = std::min(ret, (std::uint64_t)m_data[left + 1]);
        }

        if (right & 1) {
          if (right >= off)
            ret = std::min(ret, (std::uint64_t)m_tab[right - off - 1]);
          else ret = std::min(ret, (std::uint64_t)m_data[right - 1]);
        }
